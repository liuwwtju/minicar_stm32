...
{ 
    ...
    {	
			...
			//获取编码器数据，即车轮实时速度MOTOR_A.Encoder ，并转换位国际单位
			...
			if(Check==0) //If self-check mode is not enabled //如果没有启动自检模式
			{
				...
				//CAN、串口1、串口3(ROS)、串口5控制直接得到三轴目标速度，根据三轴速度逆解除四个车轮的目标速度MOTOR_A/B/C/D.Target
				...
				if(Turn_Off(Voltage)==0) 
				 { 			
           		//Speed closed-loop control to calculate the PWM value of each motor, 
					 ...
					 MOTOR_A.Motor_Pwm=Incremental_PI_A(MOTOR_A.Encoder, MOTOR_A.Target); //已知实时速度和目标速度求各个电机的pwm值
